    ...
    <section class="detail-container">
        <div class="detail-grid">
            ...
            <div class="detail-sidebar-card reveal" style="transition-delay: 0.2s">
                ...
                <button type="button" 
                        class="btn-booking" 
                        style="background: transparent; border: 1px solid var(--accent); color: var(--near-black); margin-top: 0.8rem; cursor: pointer;"
                        onclick="DiorealCart.addItem({
                            id: 'otel-{{ $otel->id }}',
                            type: 'Oteller',
                            name: '{{ addslashes($otel->name['tr'] ?? $otel->name['en'] ?? 'Otel') }}',
                            price: 28500,
                            quantity: 1,
                            image: '{{ asset($otel->hero_image ?? ($otel->cover_image ?? 'foto.img/hero_4k.jpg')) }}',
                            details: 'Lüks Konaklama • Special Rate'
                        }); alert('Tebrikler! {{ addslashes($otel->name['tr'] ?? 'Otel') }} sepetinize eklendi.');">
